...
public final class WebSecurityBundleTests {
    ...
    public void testDefaultFiltersApplied() throws Exception {
        ...
        verify(this.environment.servlets()).addFilter(anyString(), isA(JerseyAwareWebSecurityFilter.class));
        verify(this.environment.servlets(), never()).addFilter(anyString(), isA(CrossOriginFilter.class));
    }

    @Test
    public void testCorsFiltersAppliedWhenEnabled() throws Exception {
        WebSecurityBundle bundle = new WebSecurityBundle();
    ...
    @Test
    public void testCorsFiltersNotAppliedWhenDisabled() throws Exception {
        WebSecurityBundle bundle = new WebSecurityBundle();
        WebSecurityConfiguration webSecurityConfig = WebSecurityConfiguration.builder()
    ...
    public void testDefaultPropertyMap() throws Exception {
        ...
        assertEquals(
                Boolean.toString(CorsConfiguration.DEFAULT_ALLOW_CREDENTIALS),
                captured.get(CrossOriginFilter.ALLOW_CREDENTIALS_PARAM));
        assertEquals(CorsConfiguration.DEFAULT_ALLOWED_HEADERS, captured.get(CrossOriginFilter.ALLOWED_HEADERS_PARAM));
        assertEquals(CorsConfiguration.DEFAULT_ALLOWED_METHODS, captured.get(CrossOriginFilter.ALLOWED_METHODS_PARAM));
